      ...
      <div style={{ marginBottom:20 }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          {Array.from({ length: Math.max(sets, completedForCurrent) }).map((_, i) => {
            ...
            return (
              <div key={i} onClick={() => done && setEditingSet(isEditing ? null : { exName: ex.name, setIdx: i })}
                style={{
                  ...
                  display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
                  transition:"all 0.3s", cursor: done ? "pointer" : "default", padding:4,
                }}>
                {isEditing ? (
                  <div style={{ display:"flex", flexDirection:"column", gap:3, width:"100%", padding:"0 4px" }}>
                    <input type="number" value={setInfo?.w} onChange={e => handleEditSet(ex.name,i,"w",e.target.value)}
                      style={{ background:"transparent", border:"none", color:accentColor, fontSize:11, fontWeight:700, textAlign:"center", width:"100%", outline:"none" }}/>
                    <input type="number" value={setInfo?.r} onChange={e => handleEditSet(ex.name,i,"r",e.target.value)}
                      style={{ background:"transparent", border:"none", color:"#888", fontSize:10, textAlign:"center", width:"100%", outline:"none" }}/>
                  </div>
                ) : done ? (
                  <>
                    <div style={{ fontSize:12, fontWeight:700, color:accentColor }}>{setInfo?.w} lbs</div>
                    <div style={{ fontSize:10, color:"#888" }}>×{setInfo?.r}</div>
                    <div style={{ fontSize:8, color:"#555", marginTop:1 }}>tap to edit</div>
                  </>
                ) : (
