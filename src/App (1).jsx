...
// ─── PROFILE SCREEN ───────────────────────────────────────────────────────────
const ProfileScreen = ({ user, phase, onReset, onSaveName, onSignOut, accentColor }) => {
...
export default function App() {
  ...
  // ── Auth listener — restores persisted session on app load ──────────────
  useEffect(() => {
    // getSession() reads from localStorage — restores login across app restarts
  ...
  const loadAllData = async () => {
    setDataLoading(true);
    try {
      // Load or create profile — use maybeSingle() so missing row returns null, not an error
      ...
      if (prof) {
        setProfile(prof);
        setWorkoutCount(prof.workout_count || 0);
      } else {
  ...
  const handleLogWeight = async (val) => {
    ...
    const { data } = await supabase.from("weight_logs").insert(log).select().single();
    if (data) {
      setWeightLogs(prev => [...prev, data]);
      // Update current_weight on profile
      await supabase.from("profiles").update({ current_weight: val }).eq("id", authUser.id);
      setProfile(p => ({ ...p, current_weight: val }));
    }
  };
  ...
  const handleWorkoutFinish = async (newLogs) => {
    if (newLogs.length > 0) {
      ...
      if (data) setExerciseLogs(prev => [...prev, ...data]);

      // Increment workout count
      const newCount = workoutCount + 1;
